...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY)
    {
        ...
        float center2 = PADDLE_HEIGHT / 2.0 + pong->player2.y;
        float center1 = PADDLE_HEIGHT / 2.0 + pong->player1.y;
        
        if (pong->ball.y > center2 && pong->ball.vx > 0) pong->player2.vy = PADDLE_SPEED;
        else if (pong->ball.y < center2 && pong->ball.vx > 0) pong->player2.vy = -PADDLE_SPEED;
        else pong->player2.vy = 0;
        
        if (pong->ball.y > center1 && pong->ball.vx < 0) pong->player1.vy = PADDLE_SPEED;
        else if (pong->ball.y < center1 && pong->ball.vx < 0) pong->player1.vy = -PADDLE_SPEED;
        else pong->player1.vy = 0;
